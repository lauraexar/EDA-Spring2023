...
#5. Plot the data, how should we plot it?
...
USGS.Gages <- read.csv("./Data/Raw/NWIS_SiteInfo_NE_RAW.csv", stringsAsFactors = TRUE)
...
ggplot() +
  ...
  geom_sf(data = USGSGages.sf, color='blue', size=2) + 
...
Site.Flow <- read.csv("./Data/Raw/NWIS_SiteInfo_NE_RAW.csv", stringsAsFactors = TRUE)

Site.Flow$site_no <- as.factor(Site.Flow$site_no)
...
ggplot() +
  geom_sf(data = counties_sf, aes(fill = ALAND), color = 'white')  +
  geom_sf(data = Site.Flow, aes(fill=)) + 
...
Douglas.Gages <- Douglas.County[USGSGages.sf,]

#24 Plot
ggplot() +
  geom_sf(data = counties_sf, aes(color = 'white'))  +
  geom_sf(data = Douglas.County, aes(fill = 'red')) +
  geom_sf(data = Douglas.Gages, color='blue', size=2)
