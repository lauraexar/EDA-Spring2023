...
#Shows coordinate reference system
st_crs(counties_sf) 
...
mapview(huc8_sf, col.regions='purple') + 
  mapview(huc8_sf_UN, col.regions='orange', alpha=1)

ggplot() +
  geom_sf(data = huc8_sf,fill='purple', color='gray') + 
  geom_sf(data = huc8_sf_UN,fill='orange')
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  colClasses = c('HUC_12' = 'factor')) %>%  #numeric column but treat as factor
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #creates a column with first 8 characters
  group_by(HUC_8) %>% #group by same numbers
  summarize(SumWindEnergy = sum(AvgWindEnergy)) #one rows for each that has the sum of the average wind energy
...
neuse_intersect_3 <- counties_sf_utm[neuse_sf,]
...
tri_HUCs1 <- huc8_sf_utm[triangle_counties,]
mapview(tri_HUCs1)

tri_HUCs2 <- huc8_sf_utm %>%
  st_filter(triangle_counties)
mapview(tri_HUCs2)

#Intersect the HUC_8s
tri_HUCs_clipped = huc8_sf_utm %>% 
  st_intersection(triangle_counties)
mapview(tri_HUCs_clipped)
