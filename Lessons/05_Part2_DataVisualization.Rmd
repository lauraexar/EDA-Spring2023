...
#white background, gray lines
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2)

#also white background, gray lines
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #line at point 50
  geom_hline(yintercept = 100, lty = 2) + #line at point 100
  geom_point(alpha = 0.5, size = 1.5) + #determines color and size of points
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #changes scale on the y axis
    date_breaks = "2 months", date_labels = "%b %y") + #changes breaks and label
  ylab(expression("O"[3]* " AQI Value")) + #creates a new y axis label
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #changes the angle and justification of the text
print(O3plot7)  
...
#plotting two nutrients, categorized by depth, changing shape of point based on lake
NvsP <-
...
NeonicsPlots <- 
  ...
  cale_color_viridis(option = "magma", direction = -1) +
geom_bar()
print(NeonicsPlots)
