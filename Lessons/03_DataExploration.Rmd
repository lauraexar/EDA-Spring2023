...
> Answer: When you load a package, the output shows on the console. The console shows the packages that are coming with package, for example, the package Tidyverse includes the packages tibble, tidyr, readr. 
...
> ANSWER: This file can be found in the Metadata folder, and it shows up on the R Studio screen when you click on it. The placement and information found in the file are related to best practices because not only does it provide information about the dataset and database, but it walks through the steps for how to download the data yourself from the USGS Water Data website. This makes the data download process reproducible because it would be easy for another user to download the data following these steps. 
...
> Answer: R imports blank cells in a spreadsheet as "NA". 
...
> Answer: Research questions regarding things like calculating the mean and standard deviation would make it favorable to remove NAs from a dataset, because they are not needed for those computations and may make the calculating more difficult. It would be favorable to retain the NAs in research questions that involve time series. NAs should never be removed for time series analysis, however, there are methods to replace NAs with another value. Special care should be exercised when NAs are in the middle of the data because it may be difficult to remove them.
