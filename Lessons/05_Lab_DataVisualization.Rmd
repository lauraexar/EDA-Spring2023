...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #paste brinsg together strings, paste0 does not insert spaces, paste adds a space
    )

#Print the plot
print(Plot_text)

#Save the plot, specify output file name and different parameters
ggsave(
...
glimpse(PeterPaul.chem.nutrients$month) #would also work
str(PeterPaul.chem.nutrients$month) #same as glimpse

#List unique values? 5, 6, 7, 8, 9, 10, 11, 2
...
#Try again: Plot how many Lake observations occur each month, helpful for assignment, factors are categories and factors have levels
ggplot(PeterPaul.chem.nutrients,
       aes(x=factor(month, levels=1:12, labels=month.abb))) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) + #numbers are categories, not discrete anymore
  labs(
    title = "My Plot",
    x = "Month")
...
#Tidy up the code, NEED TO FINISH FROM SLIDES
...
#Apply a ggtheme, mimicks the WSJ
surface_temp_1999_plot + 
  theme_wsj() + 
  scale_color_wsj()
...
# Color your points by year, and facet by lake in two rows., by something = x axis
