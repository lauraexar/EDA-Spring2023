...
Ozone2 <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone)) + 
   geom_boxplot() +
    labs(x="",
         y="Ozone AQI Value")
Ozone2

Ozone3 <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone)) + 
   geom_boxplot() +
...
Ozone4 <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone)) + 
...
#Exercise 2: Run a linear regression PM2.5 by Ozone. Find the p-value and R-squared value. P value = < 2.2e-16 (less than .05, so can use ozone to explain PM2.5), R-squared value = 0.1314 
...
Ozone5 <- ggplot(EPAair, aes(x=Ozone, y=PM2.5)) + 
   geom_point() +
  geom_smooth(method="lm", col="blue", se=FALSE) + #adds line
    labs(x="Ozone", y="PM2.5")
Ozone5
...
EPAairCorr <- cor(EPAair.subset) #matrix of correlation between each other
