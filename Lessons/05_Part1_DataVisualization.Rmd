...
install.packages(ggridges) #couldn't download this package
library(ggridges)
...
#the color needs to be outside of the () to change the chart color
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? Color as a function of site name
PMplot <- 
...
# Filter dataset within plot building and facet by multiple variables, subset by site name
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), #rows representing sites
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #column representing years
print(PMplot.faceted2)

# Plot true time series with geom_line, line plot taht connects the points
PMplot.line <- 
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #added trendline
  geom_abline(aes(slope = 16, intercept = 0)) #specifies slope and intercept
print(NvsP)
...
# Violin plot, box plots show summary statistics, violin plots show density plots like a rotated density plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Using a gathered dataset, easier to generate plots, has legend
Nutrientplot7 <-   
...
# Ridgeline plot, plot that shows the density of the distribution over time
Nutrientplot6 <-
