...
O3.onesample #df (related to number of observations) and p value, p value less than 0.05, so we will reject the null hypothesis, we know that population mean is less than or equal to 50
...
> There are no explanatory vairables. There's just one coefficient, which is the mean that we had before.
...
O3.twosample #reject null hypothesis that the two means are the same

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year)
summary(O3.twosample2) #year is a predictor of ozone levels
