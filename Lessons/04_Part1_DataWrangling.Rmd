...
> Add notes here: We may be interested in looking at this type of data because 
...
colnames(NTL.phys.data) #name of all of the columns that we imported
head(NTL.phys.data) #first six observations of data set
summary(NTL.phys.data) #summary for each of the columns
str(NTL.phys.data) #what is the data type of each column
dim(NTL.phys.data) #what is the dimension of the dataset
...
`==` #compare value to a specific value, like for 'if' statements
`!=` #check in values are different
`<` #check if values are less than a number
`<=` #check if values are less than or equal to a number
`>`#check if values are greater than a number
`>=` #check if values are greater than or equal to a number
`&` #aggregate multiple conditions, if you want them both to be true
`|` #aggregate multiple conditions, if you want one or the other to be true
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filtering only rows that have 0 in column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)

# Did the methods arrive at the same result? Yes, the dimensions and teh first six observations are the same.
...
summary(NTL.phys.data$lakename) #lake names with the most appearances
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #select all instances of all instances of Paul Lake or Peter Lake
...
#could exclude all other lakes to find the instances of Paul Lake and Peter Lake
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #useful for selecting based on multiple elements

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) # '&' to include both
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)# ',' to include both
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304) #can use greater than or equal and less than or equal
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #selects the values including and in between these numbers
...
TuesdayLake90_99 <- filter(TuesdayLake, year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: We want to filter by different variables, which are the columns. The rows are observations, which we don't want to filter by.
...
# Which dates, lakes, and depths have the highest temperatures? The highest temperature was on 1998-07-16 at East Long Lake at a depth of 0.5. The second highest temperature was on 2002-07-01 at Hummingbird Lake at a depth of 0. The third highest temperature was on 2002-07-19 at Hummingbird Lake at a depth of 0.
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #creates a tidier dataset with only the columns that you are interested in working with
...
```{r}
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32) #adding a new column to an existing dataset
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #better approach that will work for any size dataset
```
...
We want to keep the wrangling code separate from the analysis code because if we did everything in the same document, we would have a really long rmd file, where you could easily get lost. It's good practice to save processed data after you are done wrangling and then create a separate code file to analyze and visualize the dataset. Keeping the wrangling code separate from the analysis code will result in a cleaner, shorter r script, which is preferrable for reproducibility. 
