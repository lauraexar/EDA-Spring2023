...
**time series need to have a time component, equally spaced observations, and no missing observations, drop nas is not an option to deal with data gaps**

**time series have four components: trend, seasonality, cycles (longer duration patterns on dataset not related to any calendar event), randomness**
...
* **Autocorrelation**: Data points are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s)), self correlation of a series that gives us information about the correlation of current observations with previous observations, how variables are related over time
...
ggplot(EnoDischarge, aes(x = datetime, y = discharge.mean)) +
  geom_line() +
  labs(x = "", y = expression("Discharge (ft"^3*"/s)")) #plotting variable over time, large gap is issue for time series
```
...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) #drops na

EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01")) #creates dataset that considers earlier years

EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01")) #creates dataset that considers later years
```

## Decomposing a time series dataset

**stl and decompose function both lead to the same decomposition, tries to separate series into three other series: trend part, seasonal part, and random part (observed series - trend - seasonal)**
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365) #defines column, starting point (year month), and frequency (related to when the observation starts repeating itself, monthly data has frequency of 12, yearly is 1, daily is 365)
```
...
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3]) #creates a dataframe with the three columns (seasonal, trend, remainder)

EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
        Observed = EnoDischarge.late$discharge.mean,     
        Date = EnoDischarge.late$datetime) #adds two columns with observed data and data to components dataframe
...
**overall trend on the dataset, overall increase or decrease**
...
**monotonic- always going the same direction (either increasing or decreasing), but at different paces**
...
plot(wind_data_decomp) #know we are dealing with seasonal data

# Run SMK test, only one that deals with seasonal data
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts)

# Inspect results
wind_data_trend1
summary(wind_data_trend1) #result of the pvalue is less than 0.05, reject null hypothesis and say that we have a trend


wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2) #shows the results of the tau for each season of the year, some seasons have a stronger tendency of decrease
...
> Answer: There is a negative slope, which shows a decrease trend of average wind speed over time.
