...
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth) #irradiance water by depth
...
>  There is a strong negative correation between irradiance and depth--lower light levels at greater depth. The model explains approximately 31% of the total variance in irradiance. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing the plot screen into four cells, defnes the limits of the grids
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #removes outliers by changing the plotting window
  geom_point() 
...
```{r, fig.height = 3, fig.width = 4}
#filters dataset to eliminate outliers
...
summary(irradiance.regression2) #reduced standard error and increased r^2, so it is a much better fit

par(mfrow = c(2,2), mar=c(4,4,4,4)) #looking at all four plots for regression
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #take log of y axis
  geom_point() 
...
``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #subsetting dataset to only have observations for Paul Lake
                   tp_ug ~ depth + dissolvedOxygen) 
summary(TPregression) #relationship is different than 0
...
plot(TPregression)
par(mfrow = c(1,1))
#not a good residual series, something missing from regression analysis, some of the observations are outliers
```
...
library(corrplot) #creates correlation matrix to show correlation between all of the variables in the dataset
PeterPaulnutrients <- 
  PeterPaul.chem.nutrients %>%
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse") #thinner ovals have a higher correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #just prints the upper diagonal
```
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #call lm function and call all of the nutrients to explain the total phosphorus concentration

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) 
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug) #run linear regression with three optimal variables
summary(TPmodel)
