...
### One-way ANOVA
A one-way ANOVA is used when you have one categorical explanatory variable with more than two categories. In R, we can run the model with the function `lm` or `aov`, the latter of which will allow us to run post-hoc tests to determine pairwise differences. Reject null hypothesis, will say that one pair is not equal, will need to then run post-hoc testing to figure out which it is.

For example: Does total litter biomass differ among sites at the Niwot Ridge LTER? Is the mean of the litter biomass different for different sites in the dataset? 
```{r}
# Wrangle the data
...
#Main assumtpion: that the population follows a normal distribution, runs Shapiro test to check for this

shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) #data does not follow normal distribution
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_041"])
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) #follow a normal distribution, has p value of .40
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #follow a normal distribution, p value or .13
...
#can also do this to check for normality, the data does not follow a normal distribution
...
#ANOVA is robust against departures of equal variance
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID)

#results: reject null i.e. variances are not equal

# Format ANOVA as aov, specifies dataset, continuous variable, categorical variable
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova)

#results: reject null hypothesis i.e. difference between a pair of group means is statistically significant, doesn't tell us which pairs of meains is statistically significant
...
#Takes one main argument that should be the result of an ANOVA test
...
> X axis shows the site information and y axis shows the dry mass. Box plot for each site ID, angle text because site ID names are long. Add treatment groups to the box plots. You can see the differences in the means, all of the sites that have the a are not that different in terms of the mean. We would want to reorder the list of sites to show all of the groups together (like in the order of teh group lists) to show the changes in the level of mean. 
...
# Run a post-hoc test for pairwise differences, all of the possible combinations for the functional groups
...
```{r}
# Format as aov, interaction is significant among the variables
...
TukeyHSD(Litter.anova.2way3) #all possible combinations of the variables are being checked here

Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass)) #creates a new object that will show the interactions between functional groups and class

#Litter.test <- cbind(Litter,Litter.interaction) 

Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction) #compare the means for each of the litter interaction levels, one way ANOVA that shows the intercation of two categorical variables

Litter.groups <- HSD.test(Litter.anova.2way5, "Litter.interaction", group = TRUE)
Litter.groups #which interactions lead to means that are not statistically different
