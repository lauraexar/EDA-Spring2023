...
```{r, message = FALSE}
getwd()
library(plyr) #part of tidyverse
library(tidyverse)
library(lubridate)
...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #drops the other lakes so it only reports for Peter and Paul
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments)
class(NTL.nutrient.data.PeterPaul$sampledate) #sampledate is a factor
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")

#using the pipe method, create month column, select all columns, drop NAs on the depth column
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #transform 4 categories into 2 categories, one with column names and one with values, made more rows

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #remove NAs in concentration column
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) 

write.csv(NTL.nutrient.data.PeterPaul.gathered, row.names = FALSE, 
          file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv") #saved to processed data folder
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #transforming a longer dataset into a wider dataset
...
```{r}
#apply full join function to combine all rows and all columns for the two datasets, fills with NAs on the missing values
...
```{r}
#import files with one line of code for each file
...
#would need to rbind all of the 20 dataframes
...
```{r}
#creating a list using list.files function so that LitterFiles has information on all of teh 20 files
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles

#use a pipe to specify an order of functions to perform, reads each element on that files using read.csv function
Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
```
...
```{r}
Trap <- read.csv("./Data/Raw/NEON_NIWO_Litter_trapdata_raw.csv") #imported Trap dataset
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #joined by common columns: siteID, plotID, trapID

dim(Litter)
dim(Trap)
dim(LitterTrap)

#select a few columns of interest using a pipe
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum)

#saved dataset into processed folder
...
```{r}
#created a pipe to filter when depth is 0, group by lake name and month, filter out when three columns have NAs, want summary of some variables
...
#save summaries in the processed data folder
