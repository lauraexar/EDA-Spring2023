...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #creates a matrix of numbers to 20, with 5 rows and 4 columns 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #by row is true
matrix2
...
matrix1[4, ] #extracting values from matrices, gives back 4th row
matrix1[ , 3] #gives all rows of data in column 3
matrix1[c(12, 14)] #12th and 14th value
matrix1[c(12:14)] #gives 12th through the 14th value
matrix1[2:4, 1:3] #specify slices

cells <- c(1, 26, 24, 68) #builds a matrix
rnames <- c("R1", "R2") #creates two rows
cnames <- c("C1", "C2") #creates two columns
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) #puts it all together 
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 #builds a list with four items, each item has a name
list1[[4]]

# Data Frames ---- collection of vector, every column is a vector and has the same data type
d <- c(1, 2, 3, 4) # What type of vector? Numeric
e <- c("red", "white", "red", NA) # What type of vector? Character
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? Binary
dataframe1 <- data.frame(d,e,f) #Puts them all together (can do because the same length)
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #Updates the column names 

dataframe1[1:2,] #show all for rows 1 and 2 
...
> Answer: Lists and dataframes are listed in Data, with a drop down that shows the list of the vectors (or columns) within them. Matrices show the row of data, but doesn't have a drop down because it is just one row.

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: dataframe1\$ gives back a list of the data. 
...
HighsF <- c(64, 71, 72, 63, 51, 52, 61, 64, 67, 73)
HighsF
  
LowsF <- c(42, 49, 60, 46, 32, 32, 41, 47, 52, 54)
LowsF
...
```{r DataExercise-1d}
summary(df_Temps)
sd(df_Temps$HighsF) #sd won't work because it is looking for a vector
```
